...
export const ServiceStatus = ({ services }: ServiceStatusProps) => {
    return (
        <div className="glass-card">
            <h2 className="mb-4 text-lg font-heading font-semibold text-gray-100">Estado de Tus Servicios</h2>
            {services.length === 0 ? (
                <p className="text-gray-400">No se encontraron servicios activos.</p>
            ) : (
                    ...
                    {services.map((service) => (
                        <div key={service.id} className="p-4 border border-white/10 rounded-lg bg-black/20 hover:bg-black/30 transition-colors">
                            <div className="flex items-center justify-between mb-2">
                                <span className="text-xs font-medium text-primary uppercase tracking-wider">Servicio</span>
                                <StatusIcon status={service.status} />
                            </div>
                            <h3 className="text-lg font-bold text-white">{service.name}</h3>
                            <p className="mt-2 text-sm text-gray-400">
                                Estado: <span className="font-medium text-gray-200">{service.status}</span>
                            </p>
                            <p className="text-sm text-gray-500">
                                Renueva: Día {service.renewal_day} del mes
                            </p>
